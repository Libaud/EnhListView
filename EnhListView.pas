...
{$i pofs.inc}  { Standard defines for all Delphi Free Stuff components }
...
uses
  {$ifdef WINDOWS}Windows, CommCtrl, {$endif} Forms, Messages, Classes, Controls, ComCtrls, SysUtils,
  {$IFDEF POFS_COMPILER_4_UP} ImgList, {$ENDIF} Graphics, StdCtrls, Menus {$ifdef LCL}, LCLType{$endif};
...
{$ENDIF}
    ...
    procedure DrawItem(var Canvas: TCanvas; Index: Integer; Rect: TRect;
       ...
       {$IFDEF POFS_COMPILER_4_UP} reintroduce; overload; {$ENDIF} virtual;
...
{$ENDIF}
    function GetSmallImages: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList {$ELSE}
       TImageList {$ENDIF};
    procedure SetSmallImages(Val: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList
...
{$ENDIF}       
    ...
    property SmallImages:
       {$IFDEF POFS_COMPILER_4_UP} TCustomImageList {$ELSE} TImageList {$ENDIF}
       read GetSmallImages
...
begin
  ...
  while SearchStr <> '' do
  begin
    Offset := {$IFDEF POFS_COMPILER_2} Pos( {$ELSE} AnsiPos( {$ENDIF} Patt, SearchStr);
    if Offset = 0 then
    begin
...
begin
  ...
  { TODO -oFL : A revoir : implémentation LCL }
...
begin
  { TODO -oFL : A revoir : LCL implémentation }
  //inherited Edit(Item);
  if FAutoResort then
    Resort;
end;
...
begin
  ...
  begin
    ...
    // Modified by F. Libaud for LCL implementation
    {$ifdef LCL}
      RecreateWnd(Self);
    {$else}
      RecreateWnd;
...
begin
  ...
  try
    ...
    // Modified by F. Libaud for LCL/VCL correct implementaton
    { TODO -oFL : A revoir }
