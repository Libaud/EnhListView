...
{$IFNDEF POFS_WIN32_UP}
  { TODO -oFL : Code à revoir }
...
{$IFDEF POFS_COMPILER_4_UP}
  ImgList,
{$ENDIF}
  StdCtrls, Menus, EnhListView {$ifdef LCL}, LCLType{$endif};

const
  { This shuts up C++Builder 3 about the redefiniton being different. There
    seems to be no equivalent in C1.  Sorry. }
  {$IFDEF POFS_CPPB_3_UP}
...
{$ELSE} {$IFDEF POFS_DELPHI_4_UP}
  {$DEFINE DFS_C3D4COMMCTRL}
{$ENDIF} {$ENDIF}

{$IFNDEF POFS_C3D4COMMCTRL}
type
  {$IFNDEF UNICODE}
...
{$IFDEF POFS_TRY_BACKGROUND_IMAGE}
...
{$ENDIF}
  ...
  protected
    function GetOwner: TPersistent; {$IFDEF POFS_COMPILER_3_UP} override; {$ENDIF}
...
{$ENDIF}
    function GetStateImages: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList; {$ELSE} TImageList; {$ENDIF}
    procedure SetStateImages(Value: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList {$ELSE} TImageList {$ENDIF});
    function GetSmallImages: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList; {$ELSE} TImageList; {$ENDIF}
    procedure SetSmallImages(Value: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList {$ELSE} TImageList {$ENDIF});
...
{$ENDIF}
    ...
    property StateImages: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList {$ELSE} TImageList {$ENDIF}
       read GetStateImages
       write SetStateImages;
    // Redeclare so we can know when it changes and hook into it.
    property SmallImages: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList {$ELSE} TImageList {$ENDIF}
       read GetSmallImages
  ...
  begin
    if (StateImages <> NIL) then
      ...
      {ListView_SetCallbackMask(Handle, ListView_GetCallbackMask(Handle) or
        LVIS_STATEIMAGEMASK);}
    {if ListView_GetCallbackMask(Handle) or LVIS_STATEIMAGEMASK) then
      ListView_SetCallbackMask(Handle, );
    // Modified by F. Libaud
    {ListView_SetCallbackMask(Handle, ListView_GetCallbackMask(Handle) or
      LVIS_OVERLAYMASK);}
    {if
...
const
  API_STYLES: array[Low(TLVExtendedStyle)..High(TLVExtendedStyle)] of LPARAM = (
     ...
     {$IFDEF POFS_DELPHI} {$IFDEF POFS_DELPHI_7_UP} LVS_EX_INFOTIP {$ELSE} LVS_EX_LABELTIP {$ENDIF} {$ELSE} $00004000 {$ENDIF});
...
function TCustomExtListView.GetStateImages: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList; {$ELSE} TImageList; {$ENDIF}
begin
  // Nothing, just get it
  Result := inherited StateImages;
end;

procedure TCustomExtListView.SetStateImages(Value: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList {$ELSE} TImageList {$ENDIF});
begin
  SaveChecks;
  ...
  if StateImages <> NIL then
    ...
    {ListView_SetCallbackMask(Handle, ListView_GetCallbackMask(Handle) or
      LVIS_STATEIMAGEMASK)}
    {if ListView_GetCallbackMask(Handle) or LVIS_STATEIMAGEMASK then
      ListView_SetCallbackMask(Handle, Handle);}
  else
    { TODO -oFL : A revoir  }
    // Modified by F. Libaud
    {ListView_SetCallbackMask(Handle, ListView_GetCallbackMask(Handle) and not
      LVIS_STATEIMAGEMASK);}
    {if ListView_GetCallbackMask(Handle) and not LVIS_STATEIMAGEMASK then
      ListView_SetCallbackMask(Handle, Handle);}
  RestoreChecks;
end;

function TCustomExtListView.GetSmallImages: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList; {$ELSE} TImageList; {$ENDIF}
begin
  // Nothing, just get it
  Result := inherited SmallImages;
end;

procedure TCustomExtListView.SetSmallImages(Value: {$IFDEF POFS_COMPILER_4_UP} TCustomImageList {$ELSE} TImageList {$ENDIF});
begin
...
begin
  ...
  begin
    ...
    try
      ...
      // Modified by F. Libaud
      // for support LCL implementation
      {$ifdef LCL}
      { TODO -oFL : A coder }
      {$else}
